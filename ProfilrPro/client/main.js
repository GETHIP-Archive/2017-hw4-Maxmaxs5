...
Template.new.events({
  ...
  const strengths = target.strengths.value;
  const picture = target.picture.value;
  const pwidth = target.pwidth.value;
  const pheight = target.pheight.value;
  const hash = SHA1(firstName + lastName + new Date());
  ...
  target.age.value = '';
  target.bio.value = '';
  target.strengths.value = ' ';
  target.picture.value = '';
  target.pwidth.value = '';
  target.pheight.value = '';

  Router.go("profile", {_id: hash});
  },
});
